...
export class ReportsService {
  ...
  private async getReportForPeriod(
    ...
    // Revenue by cashier
    const revenueByCashier = transactions.reduce(
      (acc, t) => {
  ...
  async getRevenueByCategory(
    ...
    const revenueByCategory = transactions.reduce(
      (
        acc: Record<string, CategoryRevenue>,
        t,
      ) => {
        t.items.forEach((item) => {
          // Handle deleted products where item.product is null
          const categoryName =
            item.product?.category?.name || 'Deleted Products';
  ...
  async generatePDFReport(reportData: ReportData): Promise<Buffer> {
    ...
    return new Promise((resolve) => {
      ...
      doc.y = sectionY + 20;
      Object.entries(reportData.revenueByCashier).forEach(
        ([cashier, amount]) => {
          doc
            .fontSize(10)
            .font('Helvetica')
      ...
      bestSellers.forEach((item: any, index: number) => {
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .text(
            `${item.quantitySold} item - ${this.formatCurrency(item.revenue)}`,
            350,
            itemY,
      ...
      reportData.transactions.forEach((t, index: number) => {
  ...
  // ============ KASIR DAILY DETAIL REPORT ============

  async getKasirDailyDetail(
    date: string,
    userId: string,
    businessId: string,
  ) {
    ...
    // Get all transactions for this kasir on this date
    const transactions = await this.prisma.transaction.findMany({
      where: {
        userId,
        businessId,
    ...
    // Payment method breakdown aggregation
    ...
    transactions.forEach((transaction) => {
      ...
      transaction.items.forEach((item) => {
        ...
        const costPrice = Number((item as any).costPrice || 0) * quantity;
    ...
    // Convert product map to array and calculate percentages
    ...
    // Convert payment method map to array and calculate percentages
    const paymentMethodBreakdown = Array.from(paymentMethodMap.entries()).map(
      ([method, data]) => {
        ...
        // For CASH, add expectedCashFromSales
        if (method === 'CASH') {
    ...
    const profitMargin =
      totalSales > 0 ? Math.round((totalProfit / totalSales) * 100 * 10) / 10 : 0;
